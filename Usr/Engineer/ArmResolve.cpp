...
bool MechanicalArm::IK_cal()
{
	...
	double k = (p_x) * (p_x) + (p_y) * (p_y) + (p_z - z_offset - l1) * (p_z - z_offset - l1) - l2 * l2 - l3 * l3;
	double c3 = k / (2 * l2 * l3);
	if (c3>1)
	{
	...
	theta2 = atan2(p_z-z_offset - l1, sqrt((p_x) * (p_x) + (p_y) * (p_y))) + atan2(l3 * sin(theta3), l2 + l3 * cos(theta3));
